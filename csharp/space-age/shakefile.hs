...
soln_name = "SpaceAge"
test_file = joinPath [src_dir, "SpaceAge.cs"]

-- Shouldn't need to change anything below here.
src_dir   = joinPath [soln_name, soln_name]
src_file  = soln_name <.> "cs"
dll_file  = soln_name <.> "dll"
dll_path  = joinPath [src_dir, "bin", "Debug"]
dll_full  = joinPath [dll_path, dll_file]
soln_full = joinPath [src_dir <.> "sln"]
exercise  = joinPath [src_dir, src_file]
msbuild   = show $ joinPath ["C:\\", "Program Files (x86)", "MSBuild", "12.0", "Bin", "MSBuild.exe"]
nunit     = joinPath ["C:\\", "dev", "exercism", "csharp", "space-age", "SpaceAge", "packages", "NUnit.Runners.2.6.4", "tools", "nunit-console.exe"]
