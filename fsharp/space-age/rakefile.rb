...
SOLUTION_FILE = 'space-age.sln'
SOLUTION = File.join(SOLUTION_DIR, SOLUTION_FILE)

EXERCISE_DIR  = File.join(SOLUTION_DIR, 'space-age')
EXERCISE_FILE = 'SpaceAge.fs'
EXERCISE = File.join(EXERCISE_DIR, EXERCISE_FILE)
TESTS = File.join(EXERCISE_DIR, 'SpaceAgeTest.fs')

OUTPUT_DIR  = File.join(EXERCISE_DIR, 'bin', 'Debug')
OUTPUT_FILE = 'space_age.dll'
