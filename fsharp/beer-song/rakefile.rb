...
SOLUTION_FILE = 'beer-song.sln'
SOLUTION = File.join(SOLUTION_DIR, SOLUTION_FILE)

EXERCISE_DIR  = File.join(SOLUTION_DIR, 'beer-song')
EXERCISE_FILE = 'BeerSong.fs'
EXERCISE = File.join(EXERCISE_DIR, EXERCISE_FILE)
TESTS = File.join(EXERCISE_DIR, 'BeerSongTest.fs')

OUTPUT_DIR  = File.join(EXERCISE_DIR, 'bin', 'Debug')
OUTPUT_FILE = 'beer_song.dll'
